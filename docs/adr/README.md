# Architectural Decision Records

Numbered, immutable once accepted; superseded ADRs are marked, never deleted.

| ADR | Title | Status |
|---|---|---|
| [ADR-001](ADR-001-physics-stack-build-vs-buy.md) | Physics & simulation stack: build vs buy | Accepted (Phase 1) |
| [ADR-002](ADR-002-integration-strategy.md) | Trajectory integration strategy | Accepted (Phase 1) |
| [ADR-003](ADR-003-agent-architecture.md) | Agent & engine architecture for Monte Carlo throughput | Accepted (Phase 2) |
| [ADR-004](ADR-004-routine-spec.md) | Routine Spec `rs/1.0` and compile-to-SimProgram contract | Accepted (Phase 2) |
| [ADR-005](ADR-005-data-platform-and-xg.md) | Data platform & the xG ↔ engine integration | Accepted (Phase 4) |
| [ADR-006](ADR-006-routine-optimizer.md) | System B: the routine optimizer (Optuna over the pure objective) | Accepted (Phase 5) |
| [ADR-007](ADR-007-api-workbench-and-persistence.md) | API & Scenario Workbench: ports-and-adapters, async jobs, real squads | Accepted (Phase 6) |
| [ADR-008](ADR-008-optimization-surface-and-3d-replay.md) | Read-only optimization surface, CRN compare, on-demand 3D replay | Accepted (Phase 7) |
