...
| O-2 | Optimization over a fixed template + zone grid; lead-attacker intent pinned for feasibility; raw-coordinate / **variable-arity** search excluded (arity fixed *per study*). **Widened in P8:** up to 7 attackers (kicker + 6 runners) with off-ball zones (top-of-box / half-spaces / recycle), plus a basic free-kick genome (`FreeKickGenome`). Offside lines + off-ball runner timing remain out (→ O-3) | simplification (P5; widened P8) | n/a | genome to_scenario + 7-attacker/FK determinism tests; anti-exploit bound-pinning flag |
...
| R9 | Real-squad XI selected from the multi-match mart pool by a fixed deterministic rule (1 GK by appearances + a 4/4/2 outfield ranked by `heading+delivery`, all ties by `player_id`); attribute priors can dominate outcomes | **simplification (P6, registered)** | n/a — derived attributes only, no scraped ratings | deterministic-XI tests (same marts ⇒ same XI); mitigation = report routine *classes* not precise winners |
