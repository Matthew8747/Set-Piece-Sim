...
src/
  ...
  components/workbench/
    ScenarioWorkbench.tsx    owns mode + B/S/R keys; loads scenario + real squads
    ...
    ReplayPanel.tsx          ReplayPlayer over the pitch; worst/median/best sample picker
...
- **`@restart/pitch-kit`** — the canonical 105×68 SVG `Pitch`, the `ReplayPlayer`
  ...
  `prefers-reduced-motion`), and hand-rolled SVG charts (`Histogram`, `Ecdf`,
  `KpiCard` with a CI whisker + "how?" popover). `tokens.css` holds the full
  design scale; `globals.css` imports it. Charts are plain SVG, not visx
...
- **Honest uncertainty:** every probability carries its CI; no winner badge
  without significance (there is no cross-scenario compare yet, so the rule holds
  vacuously).
